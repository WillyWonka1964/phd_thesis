...
\nomenclature[z-OpenCV]{$MOTP$}{Multiple Object Tracking Precision}
\nomenclature[z-OpenCV]{$MOTP$}{Multiple Object Tracking Accuracy}
...
\begin{figure}[htbp!]
	\centering
	\includegraphics[width=1.0\textwidth]{leukocyte_adhesion_cascade}
...
\begin{figure}[htbp!]
	...
	\begin{tikzpicture}[figurename=figure:leukocyte_rolling,
		...
		\zoombox[magnification=3,color code=red]{0.520, 0.800}
		\zoombox[magnification=3,color code=yellow]{0.637, 0.403}
		\zoombox[magnification=3,color code=green]{0.195, 0.472}
		\zoombox[magnification=3,color code=blue]{0.587, 0.202}
	\end{tikzpicture}
	\begin{tikzpicture}[figurename=figure:leukocyte_rolling_background,
		...
		\zoombox[magnification=3,color code=blue]{0.587, 0.202}
	\end{tikzpicture}
	\caption{Image (a) is a video frame obtained from a phase contrast microscope image sequence of leukocytes in an endothelial interactive adhesion assay. In the magnified images in (b) leukocytes rolling over the endothelial monolayer can be seen in the red, yellow and blue boxes, an adhering leukocyte is seen in the green box. Image (c) shows the frame after performing a CLAHE preprocessing step and background removal via frame differencing. In image (d) corresponding leukocytes are shown. Note the absence of the adhering leukocyte in the green box; because it has been stationary for more than $n$ frames it forms part of the background model.}
\end{figure}
...
The distribution of velocities of leukocytes flowing over an endothelial monolayer can be used to study interaction events associated with an inflammatory response. Determination of leukocyte velocities requires detection of leukocyte positions in each video frame (as described in section~\ref{leukocytes:analysis:detection}). In addition to this, a method is needed to link leukocytes over multiple frames. Tracking of leukocytes over multiple consecutive video frames is used to collect trajectory data, including the instantaneous velocity and acceleration, which are essential to the study of leukocyte interaction events. The linking of leukocytes into trajectories is achieved by a particle tracking algorithm.
...
\begin{figure}[htbp!]
	...
	\begin{tikzpicture}[figurename=figure:leukocyte_rolling_detected,
		...
		\zoombox[magnification=3,color code=blue]{0.587, 0.202}
	\end{tikzpicture}
	\caption{Image (a) is a video frame obtained from a phase contrast microscope image sequence of leukocytes in an endothelial interactive adhesion assay with the object detection and tracking protocol applied. The frame has been annotated with trajectories displayed in a coloured box with a number. In the magnified images in (b) leukocytes rolling over the endothelial monolayer can be seen in the red, yellow and blue boxes, an adhering leukocyte is seen in the green box.}
\end{figure}

\section{Performance evaluation}
\label{leukocytes:validation}
Three approaches were taken to establish the effectiveness of the leukocyte detection and tracking methodology, described in section~\ref{leukocytes:analysis}. An assessment of the leukocyte object classification scheme was performed, with the additional capability to perform parameter optimisation. Further to this, the accuracy of the tracking protocol was assessed with reference to a manual set of tracked leukocytes. Finally, the consistency of biological results was compared with results achieved prior, using a manual quantification method.

\subsection{Object detection}
\label{leukocytes:validation:object_detection}
The object detection method, described in section \ref{leukocytes:analysis:detection} was validated with reference to a gold standard set of detected leukocytes. The gold standard reference set was generated from 90 frames, selected at random from 12 videos and 2 experiments. In each frame the $x$ and $y$ coordinate centroids of leukocyte objects were manually labelled, giving a total of 1078 leukocytes. In this instance the gold standard does not represent a perfect classification but is the best available approximation.
...
A high MOTP and MOTA suggest that the detection and tracking methodology presented in section~\ref{leukocytes:analysis} is well suited for the task of tracking leukocytes. The MOTP and MOTA values of 3.5 pixels and 0.92 were achieved from a video of unstimulated endothelial cells. The MOTA value may be reduced in videos with more leukocyte endothelial interaction, leading to more slow particles and therefore greater probability of occlusion of leukocytes.
